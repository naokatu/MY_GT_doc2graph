...
class DocClassificationDataset(Dataset):
    def __init__(self, graphs: list, vocab: torchtext.vocab.Vocab, node_max_token: int = 8,
        ...
        for nxg in graphs:
            ...
            # 0-1 normalize
            if g.ndata['f'].min() == g.ndata['f'].max():
                print(f"警告: 文書ID {nxg.graph['docid']} ですべてのノードの頻度が同じです: {g.ndata['f'].min()}")
                g.ndata['f'] = th.ones_like(g.ndata['f']) * 0.5
            else:
                g.ndata['f'] = (g.ndata['f'] - g.ndata['f'].min()) / (
                            (g.ndata['f']).max() - g.ndata['f'].min() + epsilon)
